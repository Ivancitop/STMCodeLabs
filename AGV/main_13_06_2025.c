...
void Motion_Sensors_Init(void) {
    ...
    GPIOA->MODER &= ~(GPIO_MODER_MODER1 | GPIO_MODER_MODER5 |
                     GPIO_MODER_MODER6 | GPIO_MODER_MODER7);
    //Habilitar para cada pin su resistencia pull up (colector abierto)
    GPIOA->PUPDR |= (GPIO_PUPDR_PUPDR0_0 | GPIO_PUPDR_PUPDR1_0 | GPIO_PUPDR_PUPDR5_0 |
    ...
    EXTI->IMR |= (1<<1) | (1<<5) | (1<<6) | (1<<7) | (1<<8) | (1<<9);//Habilitar las interrupciones para cada pin

    //Habilitar flanco de bajada y de subida para sensores de presencia
    EXTI->FTSR |= (1<<1) | (1<<5) | (1<<6) | (1<<7);
    EXTI->RTSR |= (1<<1) | (1<<5) | (1<<6) | (1<<7);
