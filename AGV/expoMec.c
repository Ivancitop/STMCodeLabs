...
#define NUM_DIFFUSE_SENSORS 4 //Sensores difusos
...
void Motion_Sensors_Init(void) {
    ...
    EXTI->IMR |= (1<<4) | (1<<5) | (1<<6) | (1<<7) | (1<<8) | (1<<9);//Habilitar las interrupciones para cada pin

    //Habilitar flanco de bajada y de subida para sensores de presencia
    EXTI->FTSR |=  (1<<4) | (1<<5) | (1<<6) | (1<<7);
    EXTI->RTSR |= (1<<4) | (1<<5) | (1<<6) | (1<<7);
...
void EXTI4_15_IRQHandler(void) {
    ...
    for(int i = 4; i <= 7; i++) {
        if(pending & (1 << i)) {//Evalua el estado de las interrupciones de todos los pines
            ...
            sensor_states.diffuse[i-4] = !(GPIOA->IDR & (1 << i));//Pone el estado del sensor en el arrglo
        }
    }
...
void Check_Sensors(void) {
	...
	sensor_states.diffuse[0] = !(GPIOA->IDR & GPIO_IDR_4) ? 1 : 0;
	sensor_states.diffuse[1] = !(GPIOA->IDR & GPIO_IDR_5) ? 1 : 0;
	sensor_states.diffuse[2] = !(GPIOA->IDR & GPIO_IDR_6) ? 1 : 0;
	sensor_states.diffuse[3] = !(GPIOA->IDR & GPIO_IDR_7) ? 1 : 0;
	...
	if(motion_state_L && !new_motion_state_L && (counterLim%2==0)) {
		PWM_SetDuty(6, 0);
	}
	else if(motion_state_L && !new_motion_state_L && (counterLim%2!=0)) {
			PWM_SetDuty(5, 0);
		}
